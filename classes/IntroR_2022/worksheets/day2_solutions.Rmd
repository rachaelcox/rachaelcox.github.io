...
1. Using the `biopsy` data set, make boxplots of `clump_thickness` for each type of `outcome`. The geom you need to use is `geom_boxplot()`.
...
2. Make the same plot again, instead using `geom_violin()`. What do you notice?
...
3. Instead of `geom_histogram()`, now use `geom_density()` and fill the area under the curves by species identity.
